...
| Capability | Details |
|---|---|
| **OKF bundle output** | Two families: `knowledge/` module cards (five dimensions, keyed by `dimension`, one directory per module with `_module.yaml`) plus top-level custom topic cards, and `content/` articles; all cards/articles carry `status` / `type` / `triggers` / `description` frontmatter |
...
```
├── knowledge/
│   ├── CLI-工具/              five dimension cards (overview · architecture · tech-stack · coding-style · setup) + _module.yaml
│   └── 生成技能/              the skill that drives the pipeline + _module.yaml
