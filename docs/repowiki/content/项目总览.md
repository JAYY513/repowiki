...
布局同 [README.md:158-169](file://README.md#L158-L169)；运行时状态另存于 `.repowiki/`，本地保存、不入库（`.gitignore` 第 5 行）。

## 核心组件

- **CLI（`bin/repowiki.mjs`）**：`init` / `scan` / `state` / `status` / `validate` 五个子命令，单文件、零依赖，Node ≥ 18（[bin/repowiki.mjs:1590-1649](file://bin/repowiki.mjs#L1590-L1649)）。`state --update` 保存行号引注索引；`status --json` 提供 `affected_pages` 与可选的锚点复核候选（[bin/repowiki.mjs:886-904](file://bin/repowiki.mjs#L886-L904)、[bin/repowiki.mjs:1099-1115](file://bin/repowiki.mjs#L1099-L1115)）。
- **生成技能（`skills/repowiki-gen/`）**：agent 的流水线指令与写作规范，产出两族页面。
- **接线（`AGENTS.md`）**：`init` 注入的管理块声明 `docs/repowiki/` 为知识源，并附读取纪律。
- **产物 bundle（`docs/repowiki/`）**：`knowledge/` 模块知识卡 + `content/` 文章，`index.md` 为唯一路由入口。
...
- 模块知识：[CLI 工具](../knowledge/CLI-工具/概述.md)、[生成技能](../knowledge/生成技能/概述.md)
- 上手：[快速开始](快速开始.md)
- 产物深入：[产物格式](产物格式.md)
